    ...
    if ((err || !user) && info) {
      switch (info.name) {
        case "TokenExpiredError": {
          let auth_token = context.args[0].headers.authorization.split(" ")[1];
          // No need to verify the token right here as the AuthGuard did that before hand
          let dec_token = <{ [p: string]: any }>jwt.decode(auth_token);

          this.userDbService.findUser({
            _id: dec_token.id
          }).then(user => {
            if (user != undefined) {
              let token_pair = user.tokens.find(x => x.client == dec_token.cli);
      ...
      switch (err) {
        ...
        case "refresh_token_inactive": {
          throw new HttpException("refresh_token_inactive", HttpStatus.UNAUTHORIZED);
        }
      }
    }
    return user;
